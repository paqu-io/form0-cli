...
import {
  ...
  validateConnectorForConfiguration,
} from '../../../utils/connector-validation.js';
import {
...
export class ConnectorManager {
  ...
  async handleCommand(args) {
    ...
    switch (subcommand.toLowerCase()) {
      ...
      case 'uninstall':
        console.log(
          colors.warning('⚠️  Uninstall is CLI-only. Exit the interactive shell and run:')
        );
  ...
  async handleConfigure(args) {
    ...
    try {
      ...
      console.log(
        colors.error(
          t('connectors.configure.failed', { name: connectorName, message: error.message })
        )
      );
    }
  }
  ...
  async handleListCommand() {
    try {
      ...
      if (installedConnectors.length === 0) {
        ...
        console.log(
          colors.textMuted('Install connectors with: npm install form0-connector-<name>')
        );
        console.log();
        return;
      }
      ...
      for (const connectorName of installedConnectors) {
        ...
        const configStatus = isConfigured
          ? isEnabled
            ? colors.success('✅ Configured & Enabled')
            : colors.warning('⚪ Configured but Disabled')
          : colors.textSecondary('⚪ Not configured');
        const loadStatus = isLoaded
          ? colors.success('✅ Loaded')
          : colors.textSecondary('⚪ Not loaded');
      ...
      const configuredButNotInstalled = Object.keys(config).filter(
        (name) => !installedConnectors.includes(name)
      );
      if (configuredButNotInstalled.length > 0) {
        ...
        configuredButNotInstalled.forEach((name) => {
          ...
          const enabled = connectorConfig.enabled
            ? colors.warning('Enabled')
            : colors.textSecondary('Disabled');
      ...
      const unconfiguredLoaded = loadedConnectors.filter(
        (name) => !config[name] && installedConnectors.includes(name)
      );
  ...
  async handleStatusCommand(args) {
    try {
      ...
      if (connectorName) {
        ...
        if (metadata) {
          console.log(
            colors.textSecondary(t('connectors.status.loadedFrom')),
            colors.value(metadata.loadedFrom)
          );
          console.log(
            colors.textSecondary(t('connectors.status.sourceType')),
            colors.value(metadata.sourceType)
          );
          console.log(
            colors.textSecondary(t('connectors.status.loadedAt')),
            colors.value(metadata.loadedAt)
          );
          const dbPath = extractDatabasePath(metadata);
          if (dbPath) {
            console.log(
              colors.textSecondary(t('connectors.status.databasePath')),
              colors.value(dbPath)
            );
          }
        }
  ...
  async handleLoadCommand(args) {
    ...
    try {
      ...
      if (healthResult.healthy) {
        ...
        console.log(
          colors.warning(`⚠️ ${t('connectors.load.healthCheckFailed')}: ${healthResult.message}`)
        );
      }
    } catch (error) {
      console.log(
        colors.error(t('connectors.load.failed', { name: connectorName, message: error.message }))
      );
    }
  }
  ...
  async handleUnloadCommand(args) {
    ...
    try {
      ...
      console.log(
        colors.error(t('connectors.unload.failed', { name: connectorName, message: error.message }))
      );
    }
  }
  ...
  async handleRemoveCommand(args) {
    ...
    try {
      ...
      console.log(
        colors.textSecondary('The package will remain installed but will not be loaded or used.')
      );
  ...
  async handleTestCommand(args) {
    ...
    try {
      ...
      console.log(
        colors.error(t('connectors.test.error', { name: connectorName, message: error.message }))
      );
    }
  }
    ...
    try {
      ...
      if (success) {
        ...
        if (healthResult.healthy) {
          ...
          console.log(
            colors.warning(
              `⚠️ ${t('connectors.reload.healthCheckFailed')}: ${healthResult.message}`
            )
          );
        }
      } else {
      ...
      console.log(
        colors.error(t('connectors.reload.error', { name: connectorName, message: error.message }))
      );
    }
  }
